...
export default function Details() {
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-theme text-white">
      <header className="sticky top-0 z-30 h-[49px] bg-background1 flex items-center justify-center">
        ...
        <h1 className="text-[17px] text-text">Notification</h1>
      </header>

      <div className="px-[15px] pt-[14px]">
        <div className="h-[39px] rounded-t-[7px] bg-background1 p-[5px] flex overflow-hidden">
          <button
          ...
          <div className="mt-[14px]">
            {notifications.length > 0 ? (
              notifications.map((item) => (
                <div
                  key={item.id}
                  className="w-full rounded-[5px] bg-background1 px-[11px] py-[10px]"
                >
                  ...
                  <p className="text-[12px] text-[#8292cb] ">{item.date}</p>

                  <p className="text-[12px] leading-[16px] text-[#b8c1e3] mt-[12px]">
                    {item.message}
                  </p>
                </div>
              ))
        ...
        {activeTab === "Information" && (
          <div className="mt-[16px]">
            {informations.map((item) => (
              <div
                key={item.id}
                className="w-full rounded-lg bg-background1 px-[16px] py-[16px] mb-[16px]"
              >
                <div className="flex items-center gap-[8px]">
                  ...
                  <h2 className="text-[15px] leading-[21px] font-normal text-white">
                    {item.title}
                  </h2>
                </div>

                <p className="mt-[13px] text-[12px] leading-[16px] text-[#b9c2e5]">
                  {item.message}
                </p>

                <p className="mt-[14px] text-[13px] leading-[17px] text-[#8292cb]">
                  {item.date}
                </p>
              </div>
            ))}
